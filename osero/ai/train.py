...
if os.path.exists(MODEL_PATH):
    ...
    fine_tune = False
    if OseroBoard.BOARD_SIZE == 8:
        model = network.create_model8()
    elif OseroBoard.BOARD_SIZE == 6:
        model = network.create_model6()
    else:
        print("invalid OseroBoard.BORD_SIZE:", OseroBoard.BOARD_SIZE)
        exit(-1)
...
for i in range(1, 99999999):
    ...
    for j in range(1, 9999999999):
        ...
        for c, r in zip(X_color[::4][0:40], y_R.reshape([-1,4])):
            print("{0}: {1}".format("B" if c==1 else "W", r))
        ...
        for k, crow in enumerate(zip(test_color[::4], Rs.reshape([-1, 4]))):
            c, row = crow
            print("{0:2d}: {1}: {2}".format(k, "B" if c==1 else "W", row))
