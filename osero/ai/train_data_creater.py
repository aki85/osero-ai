...
class TrainDataCreator:
    ...
    def get_partial(self, record):
        try:
            ...
            return np.zeros([0, OseroBoard.board_size, OseroBoard.board_size]),\
                   np.zeros([0, 1]),\
                   np.zeros([0, OseroBoard.board_size, OseroBoard.board_size]),\
                   np.zeros([0, 1])
    ...
    def create_train_data(self, records, start):
        ...
        record_boards = np.concatenate(record_boards).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        record_colors = np.concatenate(record_colors).reshape([-1, 1])
        record_actions = np.concatenate(record_actions).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
    ...
    def __create_train_data(self, boards, colors, actions, rs):
        ...
        # predict
        pred_boards = np.array(pred_boards).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        pred_colors = np.array(pred_colors).reshape([-1, 1])
        pred_actions = np.array(pred_actions).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        ...
        # reshape
        X_boards = np.array(X_boards).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        X_colors = np.array(X_colors).reshape([-1, 1])
        X_actions = np.array(X_actions).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        y_Rs = Rs.reshape([-1, 1])
    ...
    def load_test_record(self, path):
        ...
        for board, action in zip(npz["X_board"], npz["X_action"]):
            ...
            actions.append(a)
        test_board = np.array(boards).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
        test_color = np.hstack([npz["X_color"]]*4).reshape([-1, 1])
        test_action = np.array(actions).reshape([-1, OseroBoard.board_size, OseroBoard.board_size])
